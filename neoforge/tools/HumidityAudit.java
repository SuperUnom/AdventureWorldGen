...
public class HumidityAudit {
 public static void main(String[] args)throws Exception {
  ...
  var plan=new PlanV2Codec().decode(Files.readAllBytes(Path.of(args[0])),new ContentId("adventureworldgen:default"),"audit",config);
  ...
  for(int z=-3000;z<=3000;z+=8)for(int x=-3000;x<=3000;x+=8) {
   ...
   if(!plan.climate().allowsEnvironment(id,qx,qz,sample))throw new AssertionError("illegal runtime biome "+id+" at "+qx+","+qz);
