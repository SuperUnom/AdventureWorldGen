...
public class DemandPlannerAudit {
 ...
 static void renderEnvironment(Path out,long seed,AdventureWorldConfig c,MacroTerrain terrain,ClimatePlan climate)throws Exception {
  ...
  for(int z=0;z<780;z++)for(int x=0;x<780;x++) {
   ...
   if(s.wet()){tc=lc=s.waterKind()==WaterKind.OCEAN?0x193C57:0x4E9BBC;}
    ...
    int category=switch(climate.typeAt(wx,wz,s)){case COLD->0x7ABBDC;case MEDIUM->0x89B872;case HOT->0xDFAC62;};
  ...
  label(temp,"Continuous temperature: blue (0) -> green (5) -> amber (10)", "Target C/M/H: "+percent(climate.targetRatios())+" | Actual: "+percent(climate.actualRatios()));
  ...
  label(bands,"COLD: blue    MEDIUM: green    HOT: amber", "Classification used for configuration; biome scores use continuous temperature.");
 ...
 static void renderBiomes(Path out,long seed,AdventureWorldConfig c,GeneratedAdventurePlan plan)throws Exception {
  ...
  for(var id:c.biomes().filler()){g.setColor(new Color(color(id,c)));g.fillRect(838,85+row*25,15,15);g.setColor(Color.WHITE);g.drawString(id.value().replace("minecraft:",""),862,97+row*25);row++;}
  g.dispose();label(im,"White dots: required biome / structure anchors | 4-block query blend", "Generated from current code and frozen terrain; not an in-game screenshot.");ImageIO.write(im,"png",out.resolve(seed+"-biomes.png").toFile());
  var legend=new StringBuilder();for(var id:c.biomes().filler())legend.append(id+" #"+String.format("%06x",color(id,c)&0xffffff)+"\n");Files.writeString(out.resolve(seed+"-legend.txt"),legend);
 }
 static int color(ContentId id,AdventureWorldConfig c) {
  ...
  return switch(n.replace("minecraft:","")) {
   case "plains"->0x8DB765;case "forest"->0x367E44;case "flower_forest"->0x8ABF9C;
 ...
 static String percent(double[] a){return String.format(java.util.Locale.ROOT,"%.1f / %.1f / %.1f %%",a[0]*100,a[1]*100,a[2]*100);}
