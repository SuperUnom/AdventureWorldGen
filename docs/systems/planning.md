...
| 失败 | 配置冲突、不支持内容、资源容量不足、当前搜索域无合法落点、执行或终检错误 |

`area.max` 是每个需求的硬上限，`area.target` 不是达到后立即停止的上限。
群系分配中 `area.min` 可在合法供给不足时放宽；普通需求甚至可缺席并报告零面积。
出生与必需结构仍须有可用落点。`TerrainCapacitySolver` 对普通最低面积采用相同放宽语义；
必需需求仍要求可用种子容量，最终环境、数量和间距检查保持硬约束。
...
合并会收窄共同候选。共同候选的粗供给为零时，恢复来源成员分别分配；选种失败时也可只拆分失败组并保留此前归属。
粗层未发现供给不能证明细层无解。
这不是全局回溯，也不承诺保留未合并方案的全部可解性。
...
`PlanningAtlas` 使用世界对齐 tile 的只读原始类型数组，直接调用自然地形公式，保留连续查询语义。
同一 tile 的并发缺失共享构建任务；已完成数据可以回收重算。`TiledBitField` 区分未确认和已计算真假。
细候选按 tile 流式产生，不建立全大陆细候选对象目录。粗层否定不代替细层验证。

`PlanningExecution` 默认保留一个逻辑处理器，按每任务工作内存估算约束并发；结果按任务编号收集。
底图、成本节点、合法域和局部栈道候选可以并行；第三方 adapter 回调仍按稳定顺序串行。
静态合法域和动态 ownership 分离。`SupplyRegionGraph` 保存每种群系的连通分量，共享预留单元同时扣减所有重叠合法域，
避免分别统计后重复承诺；未知细域仍可参与局部细化。
...
操作额度统计实际群系/单元合法性计算；每批耗尽发送进度并继续已有 frontier，不因操作次数失败。
内存、输出单元与有限搜索域仍有各自边界，不静默放宽准入。
...
必需结构在原 carrier 无合法锚点时尝试其他完整兼容 carrier，保留允许集合、来源角色与等级跨度；
失败不会自动搬迁整套已分配群系。
