...
| 带类型及高程的道路节点、三维中心线、分层施工区间、楼梯朝向、支撑、矩形结构避让范围和失败诊断 | 恢复道路几何与索引，不重新寻路 |
| 出生坐标 | 生命周期使用同一出生结果 |
...
以及显式成本/侵蚀标记、结构预检输入身份、`PlanningPolicy.CURRENT` 的规范参数计算 SHA-256。profile ID 还会在 manifest 与 payload 中独立检查。
...
缺省接入信息规范化为统一默认值，因此省略资源与显式填写相同默认值具有相同的结构规划目录身份。
...
道路候选、验证、搜索、长度及访问硬预算通过 `PlanningPolicy.roads` 进入规范输入摘要；变更会使旧 READY 失效。
线程数、缓存策略与计算批次完成顺序不参与身份；执行现场仅在内存，不进入 READY。
气候冻结宏观场平移、温湿偏置和出生平滑校准，恢复不重新选择候选。
