...
| 字段 | 类型与必填性 | 语义 |
|---|---|---|
| `biome` | 可选 ID | 约束出生群系；会参与隐含需求展开 |
| `structure` | 可选对象 | 从指定结构实例内出生 |
| `structure.id` | 对象存在时必填 ID | 必须引用 structures 条目，且其 count.max ≥ 1 |
| `structure.spawn_point` | 对象存在时必填三数值数组 | 相对结构原点的玩家脚底坐标，按实例旋转变换 |

同时指定群系和结构时，两者都要满足；结构允许列表非空时必须包含出生群系。
结构出生将该 ID 的有效实例下界至少提高到一，不额外增加一个独立名额。
...
| `entrance` | 必填三数值数组 | 会被解析、保存为需求并参与身份；实际消费限制见 [适配器](adapters.md#limits) |

count 下界是必须实现的实例，max 是允许尝试的上界；零上界同时使该 ID 的原生生成受到抑制。
spacing 检查同 ID 实例原点的两两水平距离；只有一个实例时没有成对约束。
